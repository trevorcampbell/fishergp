...
##Use GPy's library for hyperparam optimization
...
#Gaussian kernel
def expkern(X, Y=None, gamma=None, sigma=None, diag=False):
  if diag:
    return sigma*np.ones(X.shape[0])

  if Y is None:
    Y = X

  Ynsq = (Y**2/gamma**2).sum(axis=1)
  Xnsq = (X**2/gamma**2).sum(axis=1)
  return sigma*np.exp(-0.5*(Xnsq[:, np.newaxis] + Ynsq - 2.*np.dot(X/gamma**2, Y.T)))


##Base GP class
class GP(object):

  def __init__(self, X, Y, length_scale, kernel_variance, likelihood_variance):
    self.X = X
    self.Y = Y
    self.lvar = likelihood_variance
    self.kvar = kernel_variance
    self.lsc = length_scale
    self.k = lambda U, V=None, diag=False : expkern(U, V, length_scale, kernel_variance, diag)
...
class VariationalGP(GP):
  def __init__(self, X, Y, length_scale, kernel_variance, likelihood_variance):
    self.X = X
    self.Y = Y
    self.k = GPy.kern.RBF(input_dim=X.shape[1], lengthscale=length_scale, ARD=True, variance=kernel_variance)
    self.lvar = likelihood_variance
...
class InducingGP(GP):
  ...
  def pretrain(self, subsample_idcs, ridge=1e-9):
    ...
    srgp = SubsetRegressorsGP(self.X, self.Y, self.lsc, self.kvar, self.lvar)
